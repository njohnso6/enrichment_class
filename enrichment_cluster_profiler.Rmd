...
Here we are going to set up a biomaRt. What is the purpose of that?
See all those "gene names" in your table? Those aren't reliable, and may not even be one-to-one with "canonical" genes. People come up with names all the time, so 
in databases better cataloguing systems have been created. Among the most famous
are ensembl IDs, entrez IDs. We will convert the "symbols" to ensembl IDs using a biomart. Why? Because GO terms and 
```{r initial}
...
Just to show you the easier way, here we use the orgdb. 
You just have to select which ontology you want to use.
Below you can see how to select the Molecular Function ontology.
```{r easyone}
...
Now, can you make a dotplot of the cellular component on your own.
I'll let you inspect the helpfile yourself and make your plot.
```{r easytwo}
ego_mf <- enrichGO(gene = sigOE_genes,
                universe = allOE_genes,
                keyType = "ENSEMBL",
...
Finally, we can also perform GSEA
...
Let's take some of this info and use online tools. 
Go to more on the right and select "go to working directory"
Then do the bottom
```{r output}
downOE <- sigOE %>%
  filter(log2FoldChange < -fc_threshold)
